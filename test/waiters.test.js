...
const connectionString =
  ...
  "postgresql://codex:pg123@localhost:5432/waiters_test";
...
describe("WAITERS APP", async function () {
  ...
  it("should be able to add and get the days that a Mabhozeni is working", async function () {
  ...
  it("should be able to add and get the days that a Portia is working", async function () {
  ...
  //add admin
  it("should be able to add and get admin from the database", async function () {
    const adminName = "admin";
    const adminPassword = "admin";
    await waiters.addAdmin(adminName, adminPassword);
    const admin = await waiters.getAdmin(adminName);
