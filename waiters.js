function waiterAvailability(db) {
  ...
  async function addWaiter(name) {
    ...
    const selectName = await db.manyOrNone(
      "select * from waiter where name = $1",
      [upperName]
    );
  ...
  async function getWaiter(name) {
    const upperName = name.toUpperCase();
    return await db.oneOrNone("select * from waiter where name = $1", [
      upperName,
    ]);
  }
  ...
  async function getWaiterAvailability(waiterId) {
    const waiterAvailability = await db.manyOrNone(
      "select waiter_id, day_id from theSchedule where waiter_id = $1",
      [waiterId]
    );
    const waiterDays = [];
    for (let i = 0; i < waiterAvailability.length; i++) {
      const day = waiterAvailability[i].day_id;
      const getDays = await db.manyOrNone(
  ...
  async function getIndividualWaiterDays(waiterId) {
    const [waiterDays] = await Promise.all([
      db.manyOrNone("select day_id from theSchedule where waiter_id = $1", [
        waiterId,
      ]),
    ]);
  ...
  async function resetDays(waiterId) {
    return await db.none("delete from theSchedule where waiter_id = $1", [
      waiterId,
    ]);
  }
  ...
  async function getDays(waiterId) {
    const waiterDays = await db.manyOrNone(
      "select day_id from theSchedule where waiter_id = $1",
      [waiterId]
    );
    const waiterDaysName = [];
    for (let i = 0; i < waiterDays.length; i++) {
  ...
  async function getDaysId(day) {
    const dayId = await db.manyOrNone(
      "select id from theDays where name = $1",
      [day]
    );
    return dayId[0].id;
  ...
  async function addAdmin(username, password) {
    const admin = await db.manyOrNone(
      "select * from myAdmins where username = $1",
      [username]
    );
    if (admin.length === 0) {
  ...
  async function getAdmin(username) {
    return await db.oneOrNone("select * from myAdmins where username = $1", [
      username,
    ]);
  }
  ...
  async function adminUpdateWaiterDays(waiterId, days) {
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      const getDay = await db.manyOrNone(
        "select * from theSchedule where waiter_id = $1 and day_id = $2",
        [waiterId, getDayId[0].id]
      );
      console.log(getDay);
      ...
      if (getDay.length === 0) {
        ...
        await db.none(
          "delete from theSchedule where waiter_id = $1 and day_id = $2",
          [waiterId, getDayId[0].id]
        );
      }
    }
